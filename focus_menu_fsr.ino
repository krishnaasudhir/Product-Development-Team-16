// Productivity Buddy — FSR edition
...
//
// ── Rotary encoder (KY-040 / Cylewet) wiring ─────────────────
//   A (CLK) -> Pin 2   (interrupt pin INT0)
//   GND     -> GND
//   B (DT)  -> Pin 3
//   SW      -> Pin 4   (push-button, INPUT_PULLUP)
//   GND     -> GND
//
// ── Force sensor (FSR) wiring ─────────────────────────────────
//   FSR one leg  -> 5V
//   FSR other leg -> A1  AND  a 10 kΩ resistor from A1 to GND
//   (voltage-divider: A1 reads ~0 with no weight, rises with pressure)
//
// ── PAM8403 audio amplifier wiring ───────────────────────────
//   PAM8403 VDD  -> 5V      PAM8403 GND  -> GND
//   PAM8403 SDNN -> 5V      (keeps amp enabled)
//   PAM8403 RINP -> Arduino Pin 8 (AUDIO_PIN) via 200 Ω resistor
//   PAM8403 RINN -> GND
//   PAM8403 ROUTP / ROUTN -> Speaker + / Speaker -
...
#define AUDIO_PIN 44    // tone output to PAM8403

// ── Force sensor ──────────────────────────────────────────────────────────────
#define FSR_PIN   A1
#define FSR_ON    20   // raw ADC value — above this = phone is on holder
#define FSR_OFF   8    // raw ADC value — below this = phone has been lifted
...
unsigned long pausedAt  = 0;      // millis() when timer was paused
bool          timerPaused = false;
int           prevSec   = -1;
int           prevFillW = 0;
char          prevTimeStr[8] = "";
int           prevAdjVal = -1;
bool          pauseMsgShown = false;   // tracks whether pause overlay is drawn

// ── Session stats ─────────────────────────────────────────────────────────────
int           phonePickups  = 0;
unsigned long screenTimeMs  = 0;
unsigned long liftedAt      = 0;
bool          phoneIsLifted = false;
...
bool phoneOnHolder() { return analogRead(FSR_PIN) > FSR_ON; }
bool phoneLifted()   { return analogRead(FSR_PIN) < FSR_OFF; }
...
void beepFocusDone() {
  // sound 3 — soft ascending C-E-G
  softTone(523, 150); delay(30);
  softTone(659, 150); delay(30);
  softTone(784, 300);
}

void beepBreakDone() {
  // sound 5 — soft double-ping
  softTone(880, 120); delay(80);
  softTone(880, 120);
}

void beepAllDone() {
  // sound 6 — calm four-note C-D-E-G
  softTone(523, 120); delay(30);
  softTone(587, 120); delay(30);
  softTone(659, 120); delay(30);
  softTone(784, 300);
}
...
void drawHomeItem(int y, const char* label, bool sel) {
  ...
  if (sel) { setCol(CYAN); mylcd.Fill_Rectangle(70, y+9, 77, y+59); }
...
void drawPlacePhone() {
  ...
  txtCT("Timer starts automatically", 270, 1, LGRAY);
}
...
void runSetTimes() {
  runSetFocus(); runSetBreak(); runSetSess();
  scr = HOME; drawHome();
}
...
void showPauseMsg() {
  if (!pauseMsgShown) {
    // Dim the timer digits to signal pause
    updateDigits((int)((tDurMs - (pausedAt - tStart)) / 1000UL), LGRAY);
    // Draw pause banner
    setCol(BLACK);
    mylcd.Fill_Rectangle(0, 248, 480, 270);
    txtCT("Timer paused - place phone to continue", 252, 1, ORANGE);
    pauseMsgShown = true;
  }
}
...
void animSessionDone() {
  ...
  for (int r = 8; r <= 55; r += 7) { setCol(YELLOW); mylcd.Fill_Circle(240, 110, r); delay(45); }
  setCol(BLACK);
  ...
  char buf[32]; sprintf(buf, "Session %d complete!", sessDone);
...
void animAllDone() {
  ...
  for (int i = 0; i < 8; i++) { mylcd.Fill_Screen(BLACK); txtCT("Congratulations!", 85, 3, cols[i%5]); delay(220); }
  ...
  for (int i = 0; i < 10; i++) { setCol(dc[i%5]); mylcd.Fill_Circle(dx[i],dy[i],dr[i]); delay(40); }
  txtCT("Congratulations!", 148, 3, YELLOW);
  char buf[40]; sprintf(buf, "All %d sessions complete!", sessTotal);
  txtCT(buf, 198, 2, WHITE);
  txtCT("You are amazing - great work!", 226, 2, LGRAY);
  int stSec = (int)(screenTimeMs / 1000UL);
  char stBuf[40];
  sprintf(stBuf, "Screen time: %dm %ds", stSec / 60, stSec % 60);
  txtCT(stBuf, 262, 2, PINK);
  sprintf(stBuf, "Phone pickups: %d", phonePickups);
  txtCT(stBuf, 288, 2, PINK);
  delay(5000);
}
...
void tickFocus() {
  // ── FSR pause / resume ────────────────────────────────────────────────────
  if (!timerPaused && phoneLifted()) {
    timerPaused = true;
    pausedAt    = millis();
    if (!phoneIsLifted) { phonePickups++; liftedAt = millis(); phoneIsLifted = true; }
    showPauseMsg();
    return;
  }
  if (timerPaused) {
    if (phoneOnHolder()) {
      screenTimeMs += millis() - liftedAt;
      phoneIsLifted = false;
      tStart      += millis() - pausedAt;
      timerPaused  = false;
      clearPauseMsg();
    }
...
void tickBreak() {
  // Track screen time during break (break runs regardless, but count time off holder)
  if (!phoneIsLifted && phoneLifted()) {
    phonePickups++;
    liftedAt = millis();
    phoneIsLifted = true;
  } else if (phoneIsLifted && phoneOnHolder()) {
    screenTimeMs += millis() - liftedAt;
    phoneIsLifted = false;
  }
  ...
  unsigned long elapsed = millis() - tStart;
  if (elapsed >= tDurMs) {
    if (phoneIsLifted) { screenTimeMs += millis() - liftedAt; phoneIsLifted = false; }
    updateDigits(0, GREEN);
    updateBar(tDurMs);
    delay(400);
    beepBreakDone();
...
void loop() {
  ...
  switch (scr) {
    ...
    case HOME:
      ...
      if (sel) {
        if (homeSel == 0) { sessDone = 0; phonePickups = 0; screenTimeMs = 0; phoneIsLifted = false; scr = PLACE_PHONE; drawPlacePhone(); }
        else              { runSetTimes(); }
      }
      break;

    case PLACE_PHONE:
      // Auto-start when FSR detects the phone
      if (phoneOnHolder()) {
        scr = RUN_FOCUS;
        startTimer(focusMin);
        drawFocusScreen();
